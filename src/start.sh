...
mkdir -p "$MODEL_DIR"
if [ ! -f "$MODEL" ]; then
  echo "Checkpoint not found, downloading RealVisXL V5.0 (~7GB) via hf_transfer (fast, Xet-compatible)..."
  if HF_HUB_ENABLE_HF_TRANSFER=1 python -c "
from huggingface_hub import hf_hub_download
p = hf_hub_download('SG161222/RealVisXL_V5.0', 'RealVisXL_V5.0_fp16.safetensors', local_dir='$MODEL_DIR')
print('downloaded to', p)
"; then
    mv -f "$MODEL_DIR/RealVisXL_V5.0_fp16.safetensors" "$MODEL"
    echo "Checkpoint downloaded to $MODEL"
  else
    echo "hf_transfer failed, falling back to single-connection wget..."
    if wget --no-check-certificate -q -O "$MODEL.tmp.$$" "$MODEL_URL"; then
      ...
      rm -f "$MODEL.tmp.$$" "$MODEL_DIR/RealVisXL_V5.0_fp16.safetensors"
    fi
  fi
else
